...
class QSOFit():
    ...
    def Fit(self, name = None, nsmooth = 1,and_or_mask = True,reject_badpix =True, deredden = True,wave_range = None,\
        ...
        .PL_poly_BC: array
            The combination of Powerlaw, polynomial and Balmer continuum.
        ...
        if (and_or_mask == True) and (self.and_mask is not None or self.or_mask is not None) :
            self.MaskSdssAndOr(self.lam,self.flux,self.err,self.and_mask,self.or_mask)
        if reject_badpix == True:
            self.RejectBadPix(self.lam,self.flux,self.err)
        if wave_range is not None:
            self.WaveTrim(self.lam,self.flux,self.err,self.z)
        if wave_mask is not None:
            self.WaveMsk(self.lam,self.flux,self.err,self.z)    
        if deredden == True and self.ra != -999. and self.dec != -999.:
            self.DeRedden(self.lam,self.flux,self.ra,self.dec,dustmap_path)
        ...
        if self.z < 1.16 and decomposition_host == True:
            self.DoDecomposition(self.wave,self.flux,self.err,self.path)
        else:
        ...
        #fit continuum --------------------
        self.DoContiFit(self.wave,self.flux,self.err,self.ra,self.dec,self.plateid,self.mjd,self.fiberid)
        #fit line
        if linefit == True:
            self.DoLineFit(self.wave,self.line_flux,self.err,self.f)
        ...
        if save_result == True:
            ...
            self.SaveResult(self.conti_result,self.conti_result_name,self.line_result,self.line_result_name,save_fits_path,save_fits_name)
        ...
        if plot_fig == True:
            ...
            self.PlotFig(self.ra,self.dec,self.z,self.wave,self.flux,self.err,decomposition_host,linefit,\
                         self.tmp_all,self.gauss_result,self.f_conti_model,self.f,self.all_comp_range,\
                         self.uniq_linecomp_sort,self.line_flux,save_fig_path)
        

    def MaskSdssAndOr(self,lam,flux,err,and_mask,or_mask):
        """
    ...
    def RejectBadPix(self,lam,flux,err):
        """
    ...
    def WaveTrim(self,lam,flux,err,z):
        """
    ...
    def WaveMsk(self,lam,flux,err,z):
    ...
    def DeRedden(self,lam,flux,ra,dec,dustmap_path):
    ...
    def CalculateSN(self,wave,flux):
    ...
    def DoDecomposition(self,wave,flux,err,path):
        """Decompose the host galaxy from QSO"""
        datacube = self.HostDecompose(self.wave,self.flux,self.err,self.z,self.Mi,self.npca_gal,self.npca_qso,path)
    ...
    def HostDecompose(self,wave,flux,err,z,Mi,npca_gal,npca_qso,path):
        """
    ...
    def DoContiFit(self,wave,flux,err,ra,dec,plateid,mjd,fiberid):
        ...
        f = kmpfit.Fitter(residuals = self.residuals, data = (wave[tmp_all], flux[tmp_all], err[tmp_all])) 
        ...
        if self.rej_abs==True:
            if self.poly ==True:
                tmp_conti=f.params[6]*(wave[tmp_all]/3000.0)**f.params[7] +f_poly_conti(wave[tmp_all],f.params[11:])
            else:
                tmp_conti=f.params[6]*(wave[tmp_all]/3000.0)**f.params[7]
            ind_noBAL=~np.where( (flux[tmp_all] < tmp_conti-3.*err[tmp_all]) & (wave[tmp_all] <3500.) ,True,False)
            f = kmpfit.Fitter(residuals = self.residuals, data = (wave[tmp_all][ind_noBAL], smooth(flux[tmp_all][ind_noBAL],10), err[tmp_all][ind_noBAL]))
            f.parinfo=tmp_parinfo
            f.fit(params0=pp0)
        ...
        L = self.L_conti(wave,f.params)

        # calculate MC err
        if self.MC == True and self.n_trails > 0:
            conti_para_std,all_L_std = self.conti_mc(self.wave[tmp_all], self.flux[tmp_all], self.err[tmp_all],pp0,f.parinfo,self.n_trails)


        # get conti result -----------------------------
        if self.MC == False:
            self.conti_result=np.array([ra,dec,self.plateid,self.mjd,self.fiberid,self.z,self.SN_ratio_conti,f.params[1],f.params[4],f.params[6],f.params[7],\
                               f.params[11],f.params[12],f.params[13],L[0],L[1],L[2]])
            ...
            self.conti_result=np.array([ra,dec,plateid,mjd,fiberid,self.z,self.SN_ratio_conti,f.params[1],conti_para_std[1],f.params[4],conti_para_std[4],f.params[6],conti_para_std[6],f.params[7],conti_para_std[7],\
                               f.params[11],conti_para_std[11],f.params[12],conti_para_std[12],f.params[13],conti_para_std[13],L[0],all_L_std[0],L[1],all_L_std[1],L[2],all_L_std[2]])
    ...
    def residuals(self, pp, data):
        """Continual residual function used in kmpfit"""
        xval, yval, weight = data
        return (yval - self.f_conti_all(xval,pp))/weight
    
    
    #---------MC error for continuum parameters-------------------
    def conti_mc(self,x,y,err,pp0,pp_limits,n_trails):
        ...
        for tra in range(n_trails):
            ...
            f = kmpfit.Fitter(residuals = self.residuals, data = (x, flux, err),maxiter = 50)
            f.parinfo = pp_limits
            f.fit(params0=pp0)
            all_para[:,tra] = f.params

            all_L[:,tra] = np.asarray(self.L_conti(x,f.params))
    ...
    #line function-----------
    def DoLineFit(self,wave,line_flux,err,f):
        ...
        if ind_kind_line.any() == True :
            ...
            for ii in range(ncomp):
                ...
                #----tie lines--------
                self.do_tie_line(linelist,ind_line)
                ...
                if np.sum(ind_n) > 10:
                    ...
                    line_fit = self.do_line_kmpfit(linelist,line_flux,ind_line,ind_n,nline_fit,ngauss_fit)
                    ...
                    for n in range(nline_fit):
                        for nn in range(int(ngauss_fit[n])):
                            ...
                            if self.MC == True and self.n_trails > 0:
                                gauss_name_tmp_tmp=[line_name+'_scalar',line_name+'_scalar_err',line_name+'_lambda',\
                                                    line_name+'_lambda_err',line_name+'_sigma',line_name+'_sigma_err']
                            else:
                                gauss_name_tmp_tmp=[line_name+'_scalar',line_name+'_lambda',line_name+'_sigma']
    ...
    def do_line_kmpfit(self,linelist,line_flux,ind_line,ind_n,nline_fit,ngauss_fit):
        """The key function to do the line fit with kmpfit"""
        line_fit = kmpfit.Fitter(self.residuals_line , data = (np.log(self.wave[ind_n]), line_flux[ind_n], self.err[ind_n]))  #fitting wavelength in ln space
    ...
    def do_tie_line(self, linelist,ind_line):
        """Tie line's central"""
        #--------------- tie parameter-----------
    ...
    def residuals_line(self, pp, data):
    ...
    def SaveResult(self,conti_result,conti_result_name,line_result,line_result_name,save_fits_path,save_fits_name):
    ...
    def PlotFig(self,ra,dec,z,wave,flux,err,decomposition_host, linefit,tmp_all,gauss_result,f_conti_model,f,\
               all_comp_range,uniq_linecomp_sort,line_flux,save_fig_path):
        """Plot the results"""
        self.PL_poly = f.params[6]*(wave/3000.0)**f.params[7]+f_poly_conti(wave, f.params[11:])
        ...
        plt.plot(self.wave_orig,self.flux_orig,'k',label='data')
        ...
        if decomposition_host ==True and self.decomposed == True:
            ...
            host=self.flux_orig.min()

        plt.scatter(wave[tmp_all],np.repeat(self.flux_orig.max()*1.05,len(wave[tmp_all])),color='grey',marker='o',alpha=0.5)  # plot continuum region
        ...
        plt.plot(wave,f.params[6]*(wave/3000.0)**f.params[7]+f_poly_conti(wave, f.params[11:]),color='orange',lw=2,label='conti')
        if self.decomposed == False:
            self.host=self.flux_orig.min()
        plt.ylim(min(self.host.min(),flux.min())*0.9,self.flux_orig.max()*1.1)
        ...
        if self.plot_line_name == True:
            ...
            for ll in range(len(line_cen)):
                if  wave.min() < line_cen[ll] < wave.max():
                    plt.plot([line_cen[ll],line_cen[ll]],[min(self.host.min(),flux.min()),self.flux_orig.max()*1.1],'k:')
                    plt.text(line_cen[ll]+10,1.*self.flux_orig.max(),line_name[ll],rotation = 90,fontsize =15)
