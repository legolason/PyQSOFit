...
import glob
# XXX here are three not used import: os, sys, timeit
import matplotlib
import numpy as np
...
# XXX I move these functions inside the class
class QSOFit():

    def __init__(self, lam, flux, err, z, ra=- 999., dec=-999., plateid=None,
                 mjd=None, fiberid=None, path=None, and_mask=None,
                 or_mask=None):
        """
    ...
    def Fit(self, name=None, nsmooth=1, and_or_mask=True, reject_badpix=True,
            deredden=True, wave_range=None, \
            wave_mask=None, decomposition_host=True, BC03=False, Mi=None,
            npca_gal=5, npca_qso=20, \
            Fe_uv_op=True, Fe_flux_range=None, poly=False, BC=False,
            rej_abs=False, initial_guess=None, MC=True, \
            n_trails=1, linefit=True, tie_lambda=True, tie_width=True,
            tie_flux_1=True, tie_flux_2=True, \
            save_result=True, plot_fig=True, save_fig=True, plot_line_name=True,
            plot_legend=True, dustmap_path=None, \
        ...
        XXX add an item here to calculate the flux of FeII
        ...
        # XXX to calculate the flux of FeII
        ...
        if name is None:
            if np.array(
                    [self.plateid, self.mjd, self.fiberid]).any() is not None:
                self.sdss_name = str(self.plateid).zfill(4) + '-' + str(
                    self.mjd) + '-' + str(self.fiberid).zfill(4)
            else:
        ...
        ind_gooderror = np.where((self.err != 0) & ~np.isinf(self.err), True,
                                 False)
        ...
        if (and_or_mask == True) and (
                self.and_mask is not None or self.or_mask is not None):
            self._MaskSdssAndOr(self.lam, self.flux, self.err, self.and_mask,
                                self.or_mask)
        ...
        # fit continuum --------------------
        self._DoContiFit(self.wave, self.flux, self.err, self.ra, self.dec,
        ...
        if save_result == True:
            ...
            self._SaveResult(self.conti_result, self.conti_result_name,
                             self.line_result, self.line_result_name,
        ...
        if plot_fig == True:
            ...
            self._PlotFig(self.ra, self.dec, self.z, self.wave, self.flux,
                          self.err, decomposition_host, linefit, \
                          self.tmp_all, self.gauss_result, self.f_conti_model,
                          self.conti_fit, self.all_comp_range, \
                          self.uniq_linecomp_sort, self.line_flux,
                          save_fig_path)
    ...
    def _MaskSdssAndOr(self, lam, flux, err, and_mask, or_mask):
        ...
        self.lam, self.flux, self.err = lam[ind_and_or], flux[ind_and_or], err[
            ind_and_or]
    ...
    def _WaveTrim(self, lam, flux, err, z):
        ...
        del self.lam, self.flux, self.err
        self.lam, self.flux, self.err = lam[ind_trim], flux[ind_trim], err[
            ind_trim]
        if len(self.lam) < 100:
        ...
        for msk in range(len(self.wave_mask)):
            try:
                ind_not_mask = ~np.where(
                    (lam / (1. + z) > self.wave_mask[msk, 0]) & (
                                lam / (1. + z) < self.wave_mask[msk, 1]),
                    True, False)
            except IndexError:
                raise RuntimeError(
                    "Wave_mask should be 2D array, e.g., np.array([[2000,3000],[3100,4000]]).")

            del self.lam, self.flux, self.err
            self.lam, self.flux, self.err = lam[ind_not_mask], flux[
                ind_not_mask], err[ind_not_mask]
    ...
    def _CalculateSN(self, wave, flux):
        ...
        if (wave.min() < 1350. and wave.max() > 1350.) or (
                wave.min() < 3000. and wave.max() > 3000.) or \
    ...
    def _DoDecomposition(self, wave, flux, err, path):
        ...
        datacube = self._HostDecompose(self.wave, self.flux, self.err, self.z,
        ...
        if np.sum(np.where(datacube[3, :] < 0., True, False)) > 100:
            ...
            line_mask = np.where((self.wave < 4970.) & (self.wave > 4950.) |
                                 ...
                                 (self.wave < 4111.) & (self.wave > 4091.),
                                 True, False)

            f = interpolate.interp1d(self.wave[~line_mask],
                                     datacube[3, :][~line_mask],
    ...
    def _HostDecompose(self, wave, flux, err, z, Mi, npca_gal, npca_qso, path):
        ...
        if self.BC03 == False:
            galaxy = fits.open(
                path + 'pca/Yip_pca_templates/gal_eigenspec_Yip2004.fits')
            gal = galaxy[1].data
            wave_gal = gal['wave'].flatten()
            flux_gal = gal['pca'].reshape(gal['pca'].shape[1],
                                          gal['pca'].shape[2])
        if self.BC03 == True:
            cc = 0
            flux03 = np.array([])
        ...
        if Mi is None:
            quasar = fits.open(
                path + 'pca/Yip_pca_templates/qso_eigenspec_Yip2004_global.fits')
        else:
            if -24 < Mi <= -22 and 0.08 <= z < 0.53:
                quasar = fits.open(
                    path + 'pca/Yip_pca_templates/qso_eigenspec_Yip2004_CZBIN1.fits')
            elif -26 < Mi <= -24 and 0.08 <= z < 0.53:
                quasar = fits.open(
                    path + 'pca/Yip_pca_templates/qso_eigenspec_Yip2004_DZBIN1.fits')
            elif -24 < Mi <= -22 and 0.53 <= z < 1.16:
                quasar = fits.open(
                    path + 'pca/Yip_pca_templates/qso_eigenspec_Yip2004_BZBIN2.fits')
            elif -26 < Mi <= -24 and 0.53 <= z < 1.16:
                quasar = fits.open(
                    path + 'pca/Yip_pca_templates/qso_eigenspec_Yip2004_CZBIN2.fits')
            elif -28 < Mi <= -26 and 0.53 <= z < 1.16:
                quasar = fits.open(
                    path + 'pca/Yip_pca_templates/qso_eigenspec_Yip2004_DZBIN2.fits')
            else:
                raise RuntimeError(
                    'Host galaxy template is not available for this redshift and Magnitude!')
        ...
        ind_gal = np.where(
            (wave_gal > wave_min - 1.) & (wave_gal < wave_max + 1.), True,
            False)
        ind_qso = np.where(
            (wave_qso > wave_min - 1.) & (wave_qso < wave_max + 1.), True,
            False)

        flux_gal_new = np.zeros(
            flux_gal.shape[0] * flux[ind_data].shape[0]).reshape(
            flux_gal.shape[0],
            flux[ind_data].shape[0])
        flux_qso_new = np.zeros(
            flux_qso.shape[0] * flux[ind_data].shape[0]).reshape(
            flux_qso.shape[0],
            flux[ind_data].shape[0])
        for i in range(flux_gal.shape[0]):
            fgal = interpolate.interp1d(wave_gal[ind_gal], flux_gal[i, ind_gal],
                                        bounds_error=False, fill_value=0)
            flux_gal_new[i, :] = fgal(wave[ind_data])
        for i in range(flux_qso.shape[0]):
            fqso = interpolate.interp1d(wave_qso[ind_qso], flux_qso[0, ind_qso],
                                        bounds_error=False, fill_value=0)
        ...
        flux_temp = np.vstack(
            (flux_gal_new[0:npca_gal, :], flux_qso_new[0:npca_qso, :]))
        ...
        data_cube = np.vstack(
            (wave_new, flux_new, err_new, host_flux, qso_flux))

        ind_f4200 = np.where((wave_new > 4160.) & (wave_new < 4210.), True,
                             False)
        frac_host_4200 = np.sum(host_flux[ind_f4200]) / np.sum(
            flux_new[ind_f4200])
        ind_f5100 = np.where((wave_new > 5080.) & (wave_new < 5130.), True,
                             False)
        frac_host_5100 = np.sum(host_flux[ind_f5100]) / np.sum(
            flux_new[ind_f5100])
    ...
    def _DoContiFit(self, wave, flux, err, ra, dec, plateid, mjd, fiberid):
        ...
        for jj in range(len(window_all)):
            tmp = np.where(
                (wave > window_all[jj, 0]) & (wave < window_all[jj, 1]), True,
                False)
        ...
        if self.initial_guess is not None:
            ...
            pp0 = np.array(
                [0., 3000., 0., 0., 3000., 0., 1., -1.5, 0., 15000., 0.5, 0.,
                 0., 0.])

        conti_fit = kmpfit.Fitter(residuals=self._residuals, data=(
        wave[tmp_all], flux[tmp_all], err[tmp_all]))
        tmp_parinfo = [{'limits': (0., 10. ** 10)}, {'limits': (1200., 10000.)},
                       {'limits': (-0.01, 0.01)}, \
                       {'limits': (0., 10. ** 10)}, {'limits': (1200., 10000.)},
                       {'limits': (-0.01, 0.01)}, \
                       {'limits': (0., 10. ** 10)}, {'limits': (-5., 3.)}, \
                       {'limits': (0., 10. ** 10)},
                       {'limits': (10000., 50000.)}, {'limits': (0.1, 2.)}, \
                       None, None, None]
        ...
        if self.rej_abs == True:
            if self.poly == True:
                tmp_conti = conti_fit.params[6] * (wave[tmp_all] / 3000.0) ** \
                            conti_fit.params[7] + self.F_poly_conti(
                    wave[tmp_all], conti_fit.params[11:])
            else:
                tmp_conti = conti_fit.params[6] * (wave[tmp_all] / 3000.0) ** \
                            conti_fit.params[7]
            ind_noBAL = ~np.where(
                (flux[tmp_all] < tmp_conti - 3. * err[tmp_all]) & (
                            wave[tmp_all] < 3500.), True, False)
            f = kmpfit.Fitter(residuals=self._residuals, data=(
                wave[tmp_all][ind_noBAL],
        ...
        # XXX calculate FeII flux
        Fe_flux_result = np.array([])
        Fe_flux_type = np.array([])
        Fe_flux_name = np.array([])
        if self.Fe_flux_range:
            Fe_flux_result, Fe_flux_type, Fe_flux_name = self.Get_Fe_flux(wave,
                                                                          conti_fit.params[
                                                                          :6],
                                                                          self.Fe_flux_range)

        # calculate MC err
        # XXX one more parameter for FeII flux error estimate
        if self.MC == True and self.n_trails > 0:
            conti_para_std, all_L_std, Fe_flux_std = self._conti_mc(
                self.wave[tmp_all], self.flux[tmp_all],
                self.err[tmp_all], pp0, conti_fit.parinfo, self.n_trails)

        # get conti result -----------------------------
        # XXX I add items here to make the parameters complete enough to rebuild the fitting model.
        if self.MC == False:
            # XXX here I set the plateid, mjd and fiber to int
            self.conti_result = np.array(
                [ra, dec, str(plateid), str(mjd), str(fiberid), self.z,
                 self.SN_ratio_conti, conti_fit.params[0],
                 conti_fit.params[1], conti_fit.params[2], conti_fit.params[3],
                 conti_fit.params[4], conti_fit.params[5],
                 conti_fit.params[6], conti_fit.params[7], conti_fit.params[8],
                 conti_fit.params[9], conti_fit.params[10], \
                 conti_fit.params[11], conti_fit.params[12],
            ...
            self.conti_result_name = np.array(
                ['ra', 'dec', 'plateid', 'MJD', 'fiberid', 'redshift',
                 'SN_ratio_conti', 'Fe_uv_norm', 'Fe_uv_FWHM',
                 'Fe_uv_shift', 'Fe_op_norm', 'Fe_op_FWHM', 'Fe_op_shift',
                 'PL_norm', \
                 'PL_slope', 'Blamer_norm', 'Balmer_Te', 'Balmer_Tau_e',
                 'POLY_a', 'POLY_b', 'POLY_c', 'L1350', 'L3000',
                 'L5100'])
            ...
            self.conti_result = np.array(
                [ra, dec, str(plateid), str(mjd), str(fiberid), self.z,
                 self.SN_ratio_conti, \
                 conti_fit.params[0], conti_para_std[0], conti_fit.params[1],
                 conti_para_std[1], \
                 conti_fit.params[2], conti_para_std[2], conti_fit.params[3],
                 conti_para_std[3], \
                 conti_fit.params[4], conti_para_std[4], conti_fit.params[5],
                 conti_para_std[5], \
                 conti_fit.params[6], conti_para_std[6], conti_fit.params[7],
                 conti_para_std[7], \
                 conti_fit.params[8], conti_para_std[8], conti_fit.params[9],
                 conti_para_std[9], \
                 conti_fit.params[10], conti_para_std[10], conti_fit.params[11],
                 conti_para_std[11], \
                 conti_fit.params[12], conti_para_std[12], conti_fit.params[13],
                 conti_para_std[13], L[0], \
                 all_L_std[0], L[1], all_L_std[1], L[2], all_L_std[2]])
            # XXX add type here
            self.conti_result_type = np.array(
                ['float', 'float', 'int', 'int', 'int', 'float', 'float',
                 'float', \
            ...
            self.conti_result_name = np.array(
                ['ra', 'dec', 'plateid', 'MJD', 'fiberid', 'redshift',
                 'SN_ratio_conti', 'Fe_uv_norm', \
                 'Fe_uv_norm_err', 'Fe_uv_FWHM', 'Fe_uv_FWHM_err',
                 'Fe_uv_shift', 'Fe_uv_shift_err', \
                 'Fe_op_norm', 'Fe_op_norm_err', 'Fe_op_FWHM', 'Fe_op_FWHM_err',
                 'Fe_op_shift', \
                 'Fe_op_shift_err', 'PL_norm', 'PL_norm_err', 'PL_slope',
                 'PL_slope_err', 'Blamer_norm', \
                 'Blamer_norm_err', 'Balmer_Te', 'Balmer_Te_err', 'Balmer_Tau',
                 'Balmer_Tau_err', 'POLY_a', \
                 'POLY_a_err', 'POLY_b', 'POLY_b_err', 'POLY_c', 'POLY_c_err',
                 'L1350', 'L1350_err', 'L3000', \
                 'L3000_err', 'L5100', 'L5100_err'])
        ...
        f_pl_model = conti_fit.params[6] * (wave / 3000.0) ** conti_fit.params[
            7]
    ...
    def _L_conti(self, wave, pp):
        ...
        conti_flux = pp[6] * (wave / 3000.0) ** pp[7] + self.F_poly_conti(wave,
                                                                          pp[
                                                                          11:])
        ...
        for LL in zip([1350., 3000., 5100.]):
            if wave.max() > LL[0] and wave.min() < LL[0]:
                L_tmp = np.asarray(
                    [np.log10(LL[0] * self.Flux2L(conti_flux[np.where(
                        abs(wave - LL[0]) < 5., True, False)].mean(), self.z))])
            else:
                L_tmp = np.array([-1.])
    ...
    def _f_conti_all(self, xval, pp):
        ...
        """
        f_Fe_MgII = self.Fe_flux_mgii(xval,
                                      pp[0:3])  # iron flux for MgII line region
        f_Fe_Balmer = self.Fe_flux_balmer(xval, pp[
                                                3:6])  # iron flux for balmer line region
        f_pl = pp[6] * (xval / 3000.0) ** pp[7]  # power-law continuum
        f_conti_BC = self.Balmer_conti(xval, pp[8:11])  # Balmer continuum
        f_poly = self.F_poly_conti(xval, pp[
                                         11:])  # polynormal conponent for reddened spectra
    ...
    def _conti_mc(self, x, y, err, pp0, pp_limits, n_trails):
        ...
        # XXX calculate FeII flux error
        n_Fe_flux = np.array(self.Fe_flux_range).flatten().shape[0] // 2
        all_Fe_flux = np.zeros(n_Fe_flux * n_trails).reshape(n_Fe_flux,
                                                             n_trails)
        ...
        for tra in range(n_trails):
            ...
            # XXX calculate FeII flux error
            if self.Fe_flux_range:
                Fe_flux_result, Fe_flux_type, Fe_flux_name = self.Get_Fe_flux(x,
                                                                              conti_fit.params[
                                                                              :6],
                                                                              self.Fe_flux_range)
                all_Fe_flux[:, tra] = Fe_flux_result

        # XXX a better way to give std
    ...
    def _DoLineFit(self, wave, line_flux, err, f):
        ...
        ind_neg_line = ~np.where((((wave > 2700.) & (wave < 2900.)) | (
                    (wave > 1700.) & (wave < 1970.)) | \
                                  ((wave > 1500.) & (wave < 1700.)) | (
                                              (wave > 1290.) & (wave < 1450.)) | \
                                  ((wave > 1150.) & (wave < 1290.))) & (
                                             line_flux < -err), True, False)
        ...
        if ind_kind_line.any() == True:
            ...
            uniq_linecomp, uniq_ind = np.unique(
                linelist['compname'][ind_kind_line], return_index=True)
            uniq_linecomp_sort = uniq_linecomp[
                linelist['lambda'][ind_kind_line][uniq_ind].argsort()]
            ...
            comp_result_type = np.array([])  # XXX add type here
            comp_result_name = np.array([])
            gauss_result = np.array([])
            gauss_result_type = np.array([])  # XXX add type here
            gauss_result_name = np.array([])
            all_comp_range = np.array([])
            fur_result = np.array([])
            fur_result_type = np.array([])  # XXX add type here
            fur_result_name = np.array([])

            for ii in range(ncomp):
                compcenter = allcompcenter[ii]
                ind_line = np.where(
                    linelist['compname'] == uniq_linecomp_sort[ii], True,
                    False)  # get line index
                nline_fit = np.sum(ind_line)  # n line in one complex
                linelist_fit = linelist[ind_line]
                ngauss_fit = np.asarray(linelist_fit['ngauss'],
                                        dtype=int)  # n gauss in each line

                # for iitmp in range(nline_fit):   # line fit together
                comp_range = [linelist_fit[0]['minwav'], linelist_fit[0][
                    'maxwav']]  # read complex range from table
                ...
                ind_n = np.where(
                    (wave > comp_range[0]) & (wave < comp_range[1]) & (
                                ind_neg_line == True), True, False)

                if np.sum(ind_n) > 10:
                    # call kmpfit for lines

                    line_fit = self._do_line_kmpfit(linelist, line_flux,
                                                    ind_line, ind_n, nline_fit,
                                                    ngauss_fit)

                    # calculate MC err
                    if self.MC == True and self.n_trails > 0:
                        all_para_std, fwhm_std, sigma_std, ew_std, peak_std, area_std = \
                            self._line_mc(np.log(wave[ind_n]), line_flux[ind_n],
                                          err[ind_n], self.line_fit_ini,
                                          self.line_fit_par, self.n_trails,
                                          compcenter)

                    # ----------------------get line fitting results----------------------
                    # complex parameters
                    comp_result_tmp = np.array(
                        [[linelist['compname'][ind_line][0]], [line_fit.status],
                         [line_fit.chi2_min], \
                         [line_fit.rchi2_min], [line_fit.niter],
                         [line_fit.dof]]).flatten()
                    # XXX add type here
                    comp_result_type_tmp = np.array(
                        ['str', 'int', 'float', 'float', 'int', 'int'])
                    comp_result_name_tmp = np.array(
                        [str(ii + 1) + '_complex_name',
                         str(ii + 1) + '_line_status', str(ii + 1) + \
                         '_line_min_chi2', str(ii + 1) + '_line_red_chi2',
                         str(ii + 1) + '_niter',
                         str(ii + 1) + '_ndof'])
                    comp_result = np.concatenate([comp_result, comp_result_tmp])
                    comp_result_name = np.concatenate(
                        [comp_result_name, comp_result_name_tmp])
                    comp_result_type = np.concatenate([comp_result_type,
                                                       comp_result_type_tmp])  # XXX add type here

                    # gauss result -------------

                    gauss_tmp = np.array([])
                    gauss_type_tmp = np.array([])  # XXX add type here
                    gauss_name_tmp = np.array([])

                    for gg in range(len(line_fit.params)):
                        gauss_tmp = np.concatenate(
                            [gauss_tmp, np.array([line_fit.params[gg]])])
                        if self.MC == True:
                            gauss_tmp = np.concatenate(
                                [gauss_tmp, np.array([all_para_std[gg]])])
                    gauss_result = np.concatenate([gauss_result, gauss_tmp])

                    # gauss result name -----------------
                    for n in range(nline_fit):
                        for nn in range(int(ngauss_fit[n])):
                            line_name = linelist['linename'][ind_line][
                                            n] + '_' + str(nn + 1)
                            if self.MC == True and self.n_trails > 0:
                                gauss_type_tmp_tmp = ['float', 'float', 'float',
                                                      'float', 'float',
                                                      'float']  # XXX add type here
                                gauss_name_tmp_tmp = [line_name + '_scale',
                                ...
                                gauss_type_tmp_tmp = ['float', 'float',
                                                      'float']  # XXX add type here
                                gauss_name_tmp_tmp = [line_name + '_scale',
                                                      line_name + '_centerwave',
                                                      line_name + '_sigma']
                            gauss_name_tmp = np.concatenate(
                                [gauss_name_tmp, gauss_name_tmp_tmp])
                            gauss_type_tmp = np.concatenate(
                                [gauss_type_tmp,
                                 gauss_type_tmp_tmp])  # XXX add type here
                    gauss_result_type = np.concatenate(
                        [gauss_result_type,
                         gauss_type_tmp])  # XXX add type here
                    gauss_result_name = np.concatenate(
                        [gauss_result_name, gauss_name_tmp])

                    # further line parameters ----------
                    fur_result_tmp = np.array([])
                    fur_result_type_tmp = np.array([])  # XXX add type here
                    fur_result_name_tmp = np.array([])
                    fwhm, sigma, ew, peak, area = self.line_prop(compcenter,
                                                                 line_fit.params,
                                                                 'broad')
                    ...
                    if self.MC == True:
                        fur_result_tmp = np.array(
                            [fwhm, fwhm_std, sigma, sigma_std, ew, ew_std, peak,
                             peak_std, area, area_std])
                        fur_result_type_tmp = np.concatenate(
                            [fur_result_type_tmp,
                             ['float', 'float', 'float', 'float',
                              'float', 'float', 'float', 'float',
                              'float', 'float']])  # XXX add type here
                        fur_result_name_tmp = np.array(
                            [br_name + '_whole_br_fwhm',
                             br_name + '_whole_br_fwhm_err',
                             br_name + '_whole_br_sigma',
                             br_name + '_whole_br_sigma_err', \
                             br_name + '_whole_br_ew',
                             br_name + '_whole_br_ew_err', \
                             br_name + '_whole_br_peak',
                             br_name + '_whole_br_peak_err',
                    ...
                    fur_result_type = np.concatenate([fur_result_type,
                                                      fur_result_type_tmp])  # XXX add type here
                    fur_result_name = np.concatenate(
                        [fur_result_name, fur_result_name_tmp])

                else:
                    print("less than 10 pixels in line fitting!")

            line_result = np.concatenate(
                [comp_result, gauss_result, fur_result])
            line_result_type = np.concatenate(
                [comp_result_type, gauss_result_type,
                 fur_result_type])  # XXX add type here
            line_result_name = np.concatenate(
                [comp_result_name, gauss_result_name, fur_result_name])

        else:
            line_result = np.array([])
            line_result_name = np.array([])
            # XXX with only above two value would definatly lead to a Error if code runs here.
            comp_result = np.array([])
            gauss_result = np.array([])
            ...
            print(
                "No line to fit! Pleasse set Line_fit to FALSE or enlarge wave_range!")

        self.comp_result = comp_result  # XXX for convenience of the use of rchi2 in plot
        self.gauss_result = gauss_result
        self.line_result = line_result
        self.line_result_type = line_result_type  # XXX add type here
    ...
    def _do_line_kmpfit(self, linelist, line_flux, ind_line, ind_n, nline_fit,
                        ngauss_fit):
        """The key function to do the line fit with kmpfit"""
        line_fit = kmpfit.Fitter(self._residuals_line, data=(
            np.log(self.wave[ind_n]), line_flux[ind_n],
            self.err[ind_n]))  # fitting wavelength in ln space
        line_fit_ini = np.array([])
        line_fit_par = np.array([])
        for n in range(nline_fit):
            for nn in range(ngauss_fit[n]):
                # set up initial parameter guess
                line_fit_ini0 = [0., np.log(linelist['lambda'][ind_line][n]),
                                 linelist['inisig'][ind_line][n]]
                line_fit_ini = np.concatenate([line_fit_ini, line_fit_ini0])
                # set up parameter limits
                lambda_low = np.log(linelist['lambda'][ind_line][n]) - \
                             linelist['voff'][ind_line][n]
                lambda_up = np.log(linelist['lambda'][ind_line][n]) + \
                            linelist['voff'][ind_line][n]
                sig_low = linelist['minsig'][ind_line][n]
                sig_up = linelist['maxsig'][ind_line][n]
                line_fit_par0 = [{'limits': (0., 10. ** 10)},
                                 {'limits': (lambda_low, lambda_up)},
    ...
    def _do_tie_line(self, linelist, ind_line):
        """Tie line's central"""
        # --------------- tie parameter-----------
        # XXX add a new parameter for windex and for vindex
        ...
        # XXX fixed for more params
        for iii in range(len(ind_tie_v1)):
            if ind_tie_v1[iii] == True:
                ind_tie_vindex1 = np.concatenate(
                    [ind_tie_vindex1, np.array([int(
                        np.sum(linelist['ngauss'][ind_line][0:iii]) * 3 + 1)])])
        self.delta_lambda1 = (np.log(
            linelist['lambda'][ind_line][ind_tie_v1]) - np.log(
            linelist['lambda'][ind_line][0]))[
                             1:]
        ...
        for iii in range(len(ind_tie_v2)):
            if ind_tie_v2[iii] == True:
                ind_tie_vindex2 = np.concatenate(
                    [ind_tie_vindex2, np.array([int(
                        np.sum(linelist['ngauss'][ind_line][0:iii]) * 3 + 1)])])
        self.delta_lambda2 = (np.log(
            linelist['lambda'][ind_line][ind_tie_v2]) - np.log(
            linelist['lambda'][ind_line][0]))[
                             1:]
        ...
        for iii_1 in range(len(ind_tie_f1)):
            if ind_tie_f1[iii_1] == True:
                ind_tie_findex1 = np.concatenate(
                    [ind_tie_findex1, np.array([int(
                        np.sum(linelist['ngauss'][ind_line][0:iii_1]) * 3)])])

        for iii_2 in range(len(ind_tie_f2)):
            if ind_tie_f2[iii_2] == True:
                ind_tie_findex2 = np.concatenate(
                    [ind_tie_findex2, np.array([int(
                        np.sum(linelist['ngauss'][ind_line][0:iii_2]) * 3)])])

        # get tied fvalue for case 1 and case 2
        # XXX set false option to avoid ERROR
        if np.sum(ind_tie_f1) > 0:
            self.fvalue_factor_1 = linelist['fvalue'][ind_line][ind_tie_f1][1] / \
                                   linelist['fvalue'][ind_line][ind_tie_f1][0]
        else:
            self.fvalue_factor_1 = np.array([])
        if np.sum(ind_tie_f2) > 0:
            self.fvalue_factor_2 = linelist['fvalue'][ind_line][ind_tie_f2][1] / \
                                   linelist['fvalue'][ind_line][ind_tie_f2][0]
        else:
    ...
    def _line_mc(self, x, y, err, pp0, pp_limits, n_trails, compcenter):
        ...
        all_para_1comp = np.zeros(len(pp0) * n_trails).reshape(len(pp0),
                                                               n_trails)
        all_para_std = np.zeros(len(pp0))
        ...
        for tra in range(n_trails):
            flux = y + np.random.randn(len(y)) * err
            line_fit = kmpfit.Fitter(residuals=self._residuals_line,
                                     data=(x, flux, err), maxiter=50)
            ...
            all_fwhm[tra], all_sigma[tra], all_ew[tra], all_peak[tra], all_area[
                tra] = self.line_prop(compcenter,
                                      line_fit.params,
                                      'broad')
    ...
    def line_prop(self, compcenter, pp, linetype):
        ...
        if n_gauss == 0:
            ...
            # XXX here I directly use the continuum model to avoid the inf bug of EW when the spectrum range passed in is too short
            contiflux = self.conti_fit.params[6] * (np.exp(xx) / 3000.0) ** \
                        self.conti_fit.params[7] + \
                        self.F_poly_conti(np.exp(xx),
                                          self.conti_fit.params[11:]) + \
                        self.Balmer_conti(np.exp(xx),
                                          self.conti_fit.params[8:11])
            ...
            if n_gauss > 3:
                spline = interpolate.UnivariateSpline(xx,
                                                      self.Manygauss(xx, pp[
                                                                         0:9]) - np.max(
                                                          self.Manygauss(xx, pp[
                                                                             0:9])) / 2,
                                                      s=0)
            else:
                spline = interpolate.UnivariateSpline(xx, yy - np.max(yy) / 2,
                                                      s=0)
            if len(spline.roots()) > 0:
                fwhm_left, fwhm_right = spline.roots().min(), spline.roots().max()
                fwhm = abs(
                    np.exp(fwhm_left) - np.exp(fwhm_right)) / compcenter * c

                # calculate the total broad line flux
                # area = (np.exp(xx)*yy*disp).sum()

                # calculate the line sigma and EW in normal wavelength
                # XXX here I use a func from scipy avoiding the loop and make the code simpler, and the area equal to lambda 0
                ...
                lambda2 = integrate.trapz(line_flux * line_wave * line_wave,
                                          line_wave)
                ...
                #     lambda0 = lambda0 + self.Manygauss(xx[lm],pp)*disp*np.exp(xx[lm])
                #     lambda1 = lambda1 + np.exp(xx[lm])*self.Manygauss(xx[lm],pp)*disp*np.exp(xx[lm])
                #     lambda2 = lambda2 + np.exp(xx[lm])**2*self.Manygauss(xx[lm],pp)*disp*np.exp(xx[lm])
                #
                #     ew = ew + abs(self.Manygauss(xx[lm],pp)/contiflux[lm])*disp*np.exp(xx[lm])

                sigma = np.sqrt(lambda2 / lambda0 - (
                            lambda1 / lambda0) ** 2) / compcenter * c
            else:
                fwhm, sigma, ew, peak, area = 0., 0., 0., 0., 0.
    ...
    def _residuals_line(self, pp, data):
        ...
        # ------tie parameter------------
        # XXX to match up with the modification in _do_tie_line()
        if len(self.ind_tie_vindex1) > 1:
            for xx in range(len(self.ind_tie_vindex1) - 1):
                if self.tie_lambda == True:
                    pp[int(self.ind_tie_vindex1[xx + 1])] = pp[int(
                        self.ind_tie_vindex1[0])] + self.delta_lambda1[xx]
        if len(self.ind_tie_vindex2) > 1:
            for xx in range(len(self.ind_tie_vindex2) - 1):
                if self.tie_lambda == True:
                    pp[int(self.ind_tie_vindex2[xx + 1])] = pp[int(
                        self.ind_tie_vindex2[0])] + self.delta_lambda2[xx]

        if len(self.ind_tie_windex1) > 1:
            for xx in range(len(self.ind_tie_windex1) - 1):
                if self.tie_width == True:
                    pp[int(self.ind_tie_windex1[xx + 1])] = pp[
                        int(self.ind_tie_windex1[0])]
        if len(self.ind_tie_windex2) > 1:
            for xx in range(len(self.ind_tie_windex2) - 1):
                if self.tie_width == True:
                    pp[int(self.ind_tie_windex2[xx + 1])] = pp[
                        int(self.ind_tie_windex2[0])]
        ...
        if len(self.ind_tie_findex1) > 0 and self.tie_flux_1 == True:
            pp[int(self.ind_tie_findex1[1])] = pp[int(
                self.ind_tie_findex1[0])] * self.fvalue_factor_1
        if len(self.ind_tie_findex2) > 0 and self.tie_flux_2 == True:
            pp[int(self.ind_tie_findex2[1])] = pp[int(
                self.ind_tie_findex2[0])] * self.fvalue_factor_2
        # ---------------------------------
    ...
    def _SaveResult(self, conti_result, conti_result_name, line_result,
                    line_result_name, save_fits_path, save_fits_name):
        """Save all data to fits"""
        # XXX here I add the result type and make the all_result public for convenient use.
        self.all_result = np.concatenate([conti_result, line_result])
        self.all_result_type = np.concatenate(
            [self.conti_result_type, self.line_result_type])
        self.all_result_name = np.concatenate(
            [conti_result_name, line_result_name])

        t = Table(self.all_result, names=(self.all_result_name),
                  dtype=self.all_result_type)
        t.write(save_fits_path + save_fits_name + '.fits', format='fits',
                overwrite=True)

    def _PlotFig(self, ra, dec, z, wave, flux, err, decomposition_host, linefit,
                 tmp_all, gauss_result, f_conti_model, conti_fit, \
                 all_comp_range, uniq_linecomp_sort, line_flux, save_fig_path):
        """Plot the results"""

        # XXX a lot of content are changed in this function... and due to this, in this function, I would not
        #  remark every line I modified.
        #  I notice that in some plot function, the line width are set to 2 while the default value is 1.5.
        #  I'm not quite sure where the lw=2 should be set due to it is even different depends on whether
        #  MC=true. As a result, I accept a lw=1.5 as default for all lines.

        self.PL_poly = conti_fit.params[6] * (wave / 3000.0) ** \
                       conti_fit.params[7] + self.F_poly_conti(wave,
                                                               conti_fit.params[
                                                               11:])
        ...
        # XXX in case all line fitting are failed leading to a plot error, we use np.max([self.ncomp, 1]) as ncols
        self.fig, axn = plt.subplots(nrows=2, ncols=np.max([self.ncomp, 1]),
        ...
        if self.ra == -999. or self.dec == -999.:
            ax.set_title(str(self.sdss_name) + '   z = ' + str(np.round(z, 4)),
                         fontsize=20)
        else:
            ax.set_title(
                'ra,dec = (' + str(ra) + ',' + str(dec) + ')   ' + str(
                    self.sdss_name) + '   z = ' + str(
                    np.round(z, 4)), fontsize=20)

        ax.plot(self.wave_prereduced, self.flux_prereduced, 'k', label='data')

        if decomposition_host == True and self.decomposed == True:
            ax.plot(wave, self.qso + self.host, 'pink', label='host+qso temp')
            ax.plot(wave, flux, 'grey', label='data-host')
            ax.plot(wave, self.host, 'purple', label='host')
        else:
            host = self.flux_prereduced.min()

        # XXX I remove the transparency setting here (it's meaningless in eps figure)
        ax.scatter(wave[tmp_all], np.repeat(self.flux_prereduced.max() * 1.05,
                                            len(wave[tmp_all])),
                   color='grey', marker='o')  # plot continuum region
        ...
        if linefit == True:
            ...
            line_order = {'r': 3,
                          'g': 7}  # to make the narrow line plot upper than the broad line

            temp_gauss_result = gauss_result
            for p in range(int(len(temp_gauss_result) / mc_flag / 3)):
                # XXX warn that the width used to separate narrow from broad is not exact 1200 km s-1 which would lead to wrong judgement
                if self.CalFWHM(
                        temp_gauss_result[(2 + p * 3) * mc_flag]) < 1200.:
                    color = 'g'
                    # XXX f_line_narrow is not used
                    # self.f_line_narrow = self.f_line_narrow + \
                    #                      self.Manygauss(np.log(wave), gauss_result[p * 3:(p + 1) * 3])
                else:
                    color = 'r'

                line_single = self.Onegauss(np.log(wave),
                                            temp_gauss_result[
                                            p * 3 * mc_flag:(
                                                                        p + 1) * 3 * mc_flag:mc_flag])

                ax.plot(wave, line_single + f_conti_model, color=color)
                for c in range(self.ncomp):
                    axn[1][c].plot(wave, line_single, color=color,
                                   zorder=line_order[color])
            ...
            for c in range(self.ncomp):
                ...
                f_max = line_flux[
                    np.where((wave > all_comp_range[2 * c]) & (
                                wave < all_comp_range[2 * c + 1]), True,
                             False)].max()
                f_min = line_flux[
                    np.where((wave > all_comp_range[2 * c]) & (
                                wave < all_comp_range[2 * c + 1]), True,
                ...
                axn[1][c].set_xticks([all_comp_range[2 * c],
                                      np.round((all_comp_range[2 * c] +
                                                all_comp_range[2 * c + 1]) / 2,
                                               -1),
                                      all_comp_range[2 * c + 1]])
                axn[1][c].text(0.02, 0.9, uniq_linecomp_sort[c], fontsize=20,
                               transform=axn[1][c].transAxes)
                axn[1][c].text(0.02, 0.80, r'$r\chi ^2=$' + str(
                    np.round(float(self.comp_result[c * 6 + 3]), 2)),
                               fontsize=16,
                               transform=axn[1][c].transAxes)
        ...
        #             else:
        #                 color = 'r'
        #             plt.plot(wave,self.Manygauss(np.log(wave),gauss_result[::2][p*3:(p+1)*3])+f_conti_model, color = color)
        #         plt.plot(wave,self.Manygauss(np.log(wave),gauss_result[::2])+f_conti_model,'b', label = 'line', lw = 2)
        #     else:
        #
        ...
        #             else:
        #                 color = 'r'
        #             plt.plot(wave,self.Manygauss(np.log(wave),gauss_result[p*3:(p+1)*3])+f_conti_model, color = color)
        #         plt.plot(wave,self.Manygauss(np.log(wave),gauss_result)+f_conti_model,'b', label = 'line', lw = 2)
        # plt.plot([0,0],[0,0],'r', label = 'line br')
        # plt.plot([0,0],[0,0],'g', label = 'line na')
        # plt.plot(wave,f_conti_model,'c',lw=2, label = 'FeII')
        ax.plot([0, 0], [0, 0], 'r', label='line br')
        ax.plot([0, 0], [0, 0], 'g', label='line na')
        ax.plot(wave, f_conti_model, 'c', lw=2, label='FeII')
        if self.BC == True:
            ax.plot(wave, self.f_pl_model + self.f_poly_model + self.f_bc_model,
                    'y', lw=2, label='BC')
        ax.plot(wave,
                conti_fit.params[6] * (wave / 3000.0) ** conti_fit.params[
                    7] + self.F_poly_conti(wave, conti_fit.params[11:]),
                color='orange', lw=2, label='conti')
        if self.decomposed == False:
            self.host = self.flux_prereduced.min()
        ax.set_ylim(min(self.host.min(), flux.min()) * 0.9,
                    self.flux_prereduced.max() * 1.1)
        ...
        if self.plot_line_name == True:
            line_cen = np.array(
                [6564.60, 6549.85, 6585.27, 6718.29, 6732.66, 4862.68, 5008.24,
                 4687.02, \
            ...
            line_name = np.array(
                ['', '', 'Ha+[NII]', '', '[SII]6718,6732', 'Hb', '[OIII]', \
                 'HeII4687', 'Hr', 'CaII3934', '[OII]3728', 'NeV3426', 'MgII',
                 'CIII]', \
                 'SiII1816', 'NIII]1750', 'NIV]1718', 'CIV', 'HeII1640', '', \
                 'SiIV+OIV', 'CII1335', 'Lya'])

            for ll in range(len(line_cen)):
                if wave.min() < line_cen[ll] < wave.max():
                    ax.plot([line_cen[ll], line_cen[ll]],
                            [min(self.host.min(), flux.min()),
                             self.flux_prereduced.max() * 1.1], 'k:')
                    ax.text(line_cen[ll] + 7, 1.08 * self.flux_prereduced.max(),
                            line_name[ll], rotation=90,
                            fontsize=10, va='top')
        ...
        #                 else:
        #                     color = 'r'
        #                 plt.plot(wave,self.Manygauss(np.log(wave),gauss_result[::2][p*3:(p+1)*3]), color = color)
        #             plt.plot(wave,self.Manygauss(np.log(wave),gauss_result[::2]),'b')
        #         else:
        #             for p in range(int(len(gauss_result)/3)):
        #                 if self.CalFWHM(gauss_result[3*p+1],gauss_result[3*p+2] ) < 1200.:
        #                     color = 'g'
        #                 else:
        #                     color = 'r'
        #                 plt.plot(wave,self.Onegauss(np.log(wave),gauss_result[p*3:(p+1)*3]), color = color)
        #             plt.plot(wave,self.Manygauss(np.log(wave),gauss_result),'b')
        ...
        if linefit == True:
            plt.text(0.5, -1.4, r'$\rm Rest \, Wavelength$ ($\rm \AA$)',
                     fontsize=20, transform=ax.transAxes,
                     ha='center')
            plt.text(-0.1, -0.5,
                     r'$\rm f_{\lambda}$ ($\rm 10^{-17} erg\;s^{-1}\;cm^{-2}\;\AA^{-1}$)',
    ...
    def CalFWHM(self, logsigma):
        ...
        # XXX I fix the unnecessary parameter here
    ...
    def Fe_flux_mgii(self, xval, pp):
        ...
        if np.sum(ind) > 100:
            if Fe_FWHM < 900.0:
                sig_conv = np.sqrt(910.0 ** 2 - 900.0 ** 2) / 2. / np.sqrt(
                    2. * np.log(2.))
            else:
                sig_conv = np.sqrt(Fe_FWHM ** 2 - 900.0 ** 2) / 2. / np.sqrt(
                    2. * np.log(2.))  # in km/s
    ...
    def Fe_flux_balmer(self, xval, pp):
        ...
        ind = np.where((wave_Fe_balmer > 3686.) & (wave_Fe_balmer < 7484.),
                       True, False)
    ...
    def Balmer_conti(self, xval, pp):
        ...
        lambda_BE = 3646.  # A
        bbflux = blackbody_lambda(xval, pp[
            1]).value * 3.14  # in units of ergs/cm2/s/A
    ...
    def Flux2L(self, flux, z):
        ...
        DL = cosmo.luminosity_distance(
            z).value * 10 ** 6 * 3.08 * 10 ** 18  # unit cm
    ...
    def Manygauss(self, xval, pp):
        ...
        if ngauss != 0:
            ...
            return yval
        # XXX to deal with the possible none pp input
        else:
            return np.zeros_like(xval)

    def Get_Fe_flux(self, xval, pp, ranges):
        """Calculate the flux of fitted FeII template within a given wavelength range.
        ranges: 1-D array, 2-D array
            if 1-D array was given, it should contain two parameters contain a range of wavelength. FeII flux within this range would be calculate and documented in the result fits file.
            if 2-D array was given, it should contain a series of ranges. FeII flux within these ranges would be documented respectively.
        pp: 1-D array with 3 or 6 items.
            If 3 parameters were give, function will choose a proper template (MgII or balmer) according to the range.
            If the range give excess either template, an error would be arose.
            If 6 parameters were give (recommend), function would adopt the first three for the MgII template and the last three for the balmer."""

        Fe_flux_result = np.array([])
        Fe_flux_type = np.array([])
        Fe_flux_name = np.array([])
        if np.array(ranges).ndim == 1:
            Fe_flux_result = np.append(Fe_flux_result,
                                       self._calculate_Fe_flux(xval, pp,
                                                               ranges))
            Fe_flux_name = np.append(Fe_flux_name,
                                     'Fe_flux_' + str(
                                         int(np.min(ranges))) + '_' + str(
                                         int(np.max(ranges))))
            ...
            for iii in range(np.array(self.Fe_flux_range).shape[0]):
                Fe_flux_result = np.append(Fe_flux_result,
                                           self._calculate_Fe_flux(xval, pp,
                                                                   ranges[iii]))
                Fe_flux_name = np.append(Fe_flux_name, 'Fe_flux_' + str(
                    int(np.min(ranges[iii]))) + '_' + str(
                    int(np.max(ranges[iii]))))
                Fe_flux_type = np.append(Fe_flux_type, 'float')
        else:
            raise IndexError(
                'The parameter ranges only adopts arrays with 1 or 2 dimensions.')

        return Fe_flux_result, Fe_flux_type, Fe_flux_name

    def _calculate_Fe_flux(self, xval, pp, range):
        """Calculate the flux of fitted FeII template within a given wavelength range."""
        ...
        if len(pp) == 3:
            if np.max(range) <= mgii_range[1] and np.min(range) >= mgii_range[
                0]:
                yval = self.Fe_flux_mgii(xval, pp)
            elif np.max(range) <= balmer_range[1] and np.min(range) >= \
                    balmer_range[0]:
            ...
            yval = self.Fe_flux_mgii(xval, pp[:3]) + self.Fe_flux_balmer(xval,
                                                                         pp[3:])
            if np.max(range) > balmer_range[1] or np.min(range) < mgii_range[0]:
                raise OverflowError(
                    'Make sure the range is within [1200., 7478.]!')
            elif np.max(range) > mgii_range[1] and np.min(range) < balmer_range[
                0]:
                print('Warning: The range gaven to calculate the flux of FeII pseudocontiuum (partially) \
        ...
        flux = integrate.trapz(
            yval[(xval >= np.min(range)) & (xval <= np.max(range))],
            xval[(xval >= np.min(range)) & (xval <= np.max(range))])
        return flux
